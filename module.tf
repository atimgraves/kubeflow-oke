module oke {
  ...
  tenancy_id=""
  # The home region of your tennancy - needed to congigure policies, groups and the like
  ...
  # this is creating a simple cluster, in a production environment you'll probably want better security.
  ...
  # what version of kubernetes to instal, not all kubernetes versions are supported so check in the OCI UI
  ...
  node_pools = {
    ...
    # this is a node pool for running actual applications in, e.g. the kubeflow runtime
  ...
  # to true to actually install the autoscaler, that will create an additional node pool to run the auto scaler (thus ensuring it wont't sale itself out of existence)
  # If you are using the autoscaler it is critical that you follow the instructions here https://github.com/oracle-terraform-modules/terraform-oci-oke/blob/main/docs/clusterautoscaler.adoc
  ...
  autoscaler_pools = {
    ...
    # a node pool (single node)to run it. the node pool will be specific to each kubernetes version so you can upgrade
    # clusters having different kubernetes versions in different node pools. Hence advisable to name
    # the nodepool with the kubernetes version to help you understand what's happening where
    # only 1 pool is needed at a time unless during upgrade
    asp_v126 = {}
  }

  # install the modified cloud init, this will setup the kernel modules needed by istio and also arrange for them to be installed if the node reboots
  cloudinit_nodepool_common = "./cloud-init.sh"
  # metrics server is generally useful
